     ...
     $(".wrong_per_li").click(function(event) {
         $.ajax({
             ...
             success: function(data) {
                 ...
                 $(".per_sub").html(" ");
                 for (var i = 0; i < data.length; i++) {
                     $(".per_time").append('<option value=""  data-id="' + data[i].id + '">' + data[i].name + '</option>');



                 }
                 for (var i = 0; i < data[0].subjects.length; i++) {
                     $(".per_sub").append('<option value="" data-id="' + data[0].subjects[i].subject_id + '">' + data[0].subjects[i].name + '</option>');
                 }
         ...
         $.ajax({
             ...
             success: function(data) {
                 ...
                 for (var i = 0; i < data[a].subjects.length; i++) {
                     ...
                     $(".per_sub").append('<option value="" data-id="' + data[a].subjects[i].subject_id + '">' + data[a].subjects[i].name + '</option>');
                 }
             },
             error: function() {
     ...
     $(".per_admin_li").click(function(event) {
         ...
         $.ajax({
             ...
             success: function(data) {
                 ...
                 for (var b = 0; b < data[0].subjects.length; b++) {
                     $(".per_sub").append('<option data-id="' + data[0].subjects[b].subject_id + '">' + data[0].subjects[b].name + '</option>')
                 }
                 p_students();
             },
             error: function() {
         ...
         $.ajax({
             ...
             success: function(data) {
                 ...
                 for (var i = 0; i < data[index].subjects.length; i++) {
                     $(".per_sub").append('<option data-id="' + data[index].subjects[i].subject_id + '">' + data[index].subjects[i].name + '</option>')
                 }
             },
             error: function() {
                 /* Act on the event */
         ...
         $.ajax({
             ...
             success: function(data) {
                 ...
                 for (var i = 0; i < data.length; i++) {
                     ...
                     $(".p_list_ul").append('<li id="p_a' + data[i].question_bank_id + '"><div class="p_list_main" style=""><div class="p_list_body">' + data[i].content + '</div><div class="p_list_move"><a class="p_list_body_ans" data-in="' + i + '" data-id="' + id + '"><i class="iconfont" style="margin-right:5px;"></i>查看答案和解析</a></div><div class="p_list_number"><p><i class="iconfont" style="font-size:45px;color:#fb7d8a;"></i><a>' + num + '</a></p></div><div class="p_list_form">试题来源:<a>' + data[i].source + '</a></div><p class="p_list_main_analy_show"><i class="iconfont" style="" data-id="0">&#xe624;</i></p><div class="p_list_main_analy"><p class="p_list_lable" style="margin-top: 10px;">2<a>个人得分率:<i>' + a_rate + '</i></a><a>班-年级得分率差<i>' + data[i].scoring_rate_difference + '</i></a><a>知识点:' + tag + '</a><a class="p_list_dif">难度系数:' + difficulty_body + '（实测难度：' + data[i].difficulty + '）</a></p><div class="p_list_ans"><p class="per_list_ans_top" style="color: #ccc">你的错误解答:</p><p class="per_list_ans_top"></p></div><p class="p_list_ans_be_top">错因CT:</p><div class="p_list_ans_be"><p class="p_list_ans_be01">智力因素 Z :</p><p class="p_list_ans_be02">非智力因素 FZ :<a></a></p></div><p class="p_list_ans_train_top">错题重练:</p><div class="p_list_ans_train"><p>请在此区域内纠正错题，并归纳解题心得。</p></div><p class="p_list_ans_train_top01">变式训练:</p><div class="p_list_ans_train_01"></div></div></div>');
                 }
